...
export class CommentCardService {
  ...
  private readonly _BASE_URL_COMMENT_CARD: string =
    'http://localhost:8080/commentCards';

  private commentListSubject$: BehaviorSubject<CommentCard[]> =
    new BehaviorSubject<CommentCard[]>([]);

  getCommentListFromDB(): Observable<CommentCard[]> {
    return this.http
      .get<CommentCard[]>(`${this._BASE_URL_COMMENT_CARD}/all`)
      .pipe(tap((comments) => this.commentListSubject$.next(comments)));
  }

  createComment(
    commentCard: CommentCard,
    currendUserMail: string,
    newsCardAssociated: NewsCard
    ...
    return this.http
      .post<CommentCard>(
        `${this._BASE_URL_COMMENT_CARD}/add?newsCardAssociatedId=${cardId}&senderUserMail=${senderUserMail}`,
        commentCard
      )
      .pipe(
        tap((newCommentCard) => {
  ...
  updateComment(commentCard: CommentCard): Observable<CommentCard> {
    return this.http.put<CommentCard>(
      `${this._BASE_URL_COMMENT_CARD}/update/${commentCard.id}`,
      commentCard
    );
  }
  ...
  deleteComment(id: number): Observable<void> {
    return this.http.delete<void>(
      `${this._BASE_URL_COMMENT_CARD}/delete/${id}`
    );
  }
